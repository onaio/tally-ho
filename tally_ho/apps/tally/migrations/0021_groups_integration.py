...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # First, update clerk users.
        new_group_name = 'Quality Control Archive Clerk'
        new_group = orm['auth.Group'].objects.filter(name=new_group_name).first()

        # If the new group does not exits yet, change the old group's name.
        if not new_group:
            archive_group = orm['auth.Group'].objects.filter(name='Archive Clerk').first()
            if archive_group:
                archive_group.name = new_group_name
                archive_group.save()
                new_group = archive_group
            else:
                new_group = orm['auth.Group'].objects.create(name=new_group_name)

        # If exists users for the Quality Control Clerk group, moves it.
        qc_group = orm['auth.Group'].objects.filter(name='Quality Control Clerk').first()

        if qc_group:
            for oneUser in orm['auth.User'].objects.filter(groups=qc_group):
                oneUser.groups.add(new_group)
                oneUser.groups.remove(qc_group)
                oneUser.save()

            # At the end, delete Quality Control Clerk group.
            qc_group.delete()

        # Second, update the supervisor users
        new_supervisor_name = 'Quality Control Archive Supervisor'
        new_supervisor = orm['auth.Group'].objects.filter(name=new_supervisor_name).first()

        # If the new group does not exits yet, change the old group's name.
        if not new_supervisor:
            supervisor_group = orm['auth.Group'].objects.filter(name='Archive Supervisor').first()
            if supervisor_group:
                supervisor_group.name = new_supervisor_name
                supervisor_group.save()
            else:
                new_supervisor = orm['auth.Group'].objects.create(name=new_supervisor_name)
