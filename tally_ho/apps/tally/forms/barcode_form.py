...
class BarcodeForm(forms.Form):
    ...
    validators = [
        ...
        RegexValidator(
            regex=r"^[0-9]*$", message=_("Expecting only numbers for barcodes")
        ),
    ]
    ...
    barcode_scan = forms.CharField(
        ...
        validators=validators,
        required=False,
        widget=forms.NumberInput(attrs=barcode_scan_input_attributes),
    )
        ...
        if self.is_valid():
            ...
            try:
                result_form = ResultForm.objects.get(
                    barcode=barcode or barcode_scan, tally__id=tally_id
                )
                ...
                if result_form.center and not result_form.center.active:
                    ...
                    try:
                        ...
                        raise forms.ValidationError(_("Station does not exist."))
                    else:
                        if not station.active:
                            raise forms.ValidationError(_("Station disabled."))
                        if not result_form.ballot.active:
                            raise forms.ValidationError(_("Ballot disabled."))
