$(document).ready(function () {
  ...
  const exportPdfHtml5 = doc => {
    ...
    const filterInStations = $("button[data-id='filter-in-stations']").attr('title');
    const filterInCenters = $("button[data-id='filter-in-centers']").attr('title');
    const filterInRaceTypes = $("button[data-id='filter-in-race-types']").attr('title');
    const filterOutStations = $("button[data-id='stations']").attr('title');
    const filterOutCenters = $("button[data-id='centers']").attr('title');
    const filterOutRaceTypes = $("button[data-id='filter-out-race-types']").attr('title');
    const filterInContent = `Filters Included: \nRaces: ${filterInRaceTypes} \nCenters: ${filterInCenters} \nStations: ${filterInStations}\n`;
    const filterOutContent = `Filters Excluded: \nRaces: ${filterOutRaceTypes} \nCenters: ${filterOutCenters} \nStations: ${filterOutStations}\n`;
    const filtersApplied = filterInContent + filterOutContent;
    debugger;
...
//                console.log({fontUrl});
//                const myFont = new FontFace('Arial', `url(${fontUrl})`);
//                await myFont.load();
        ...
        return {
            columns: [
                ...
                },
                {
                    alignment: 'left',
                    italics: true,
                    text: 'Candidate Results Export',
                    fontSize: 18,
                    margin: [10,0]
                },
                {
                    text: filterInContent,
                    style: 'subheader',
                    alignment: 'right',
                },
                {
                    text: filterOutContent,
                    style: 'subheader',
                    alignment: 'right',
                },
            ],
