...
class TestTurnoutByGenderReport(TestBase):
    def setUp(self):
        ...
        self.station_male = create_station(
            center=center, registrants=100, tally=self.tally,
            station_number=1, gender=Gender.MALE
        )
        self.station_female = create_station(
            center=center, registrants=120, tally=self.tally,
            station_number=2, gender=Gender.FEMALE
        )
        ...
        self.recon_form_male = create_reconciliation_form(
            ...
            user=self.user,
            number_ballots_inside_box=60, # Example voters
        ...
        self.recon_form_female = create_reconciliation_form(
            ...
            user=self.user,
            number_ballots_inside_box=80, # Example voters
            number_cancelled_ballots=0,
            number_spoiled_ballots=0,
            number_unstamped_ballots=0,
            number_unused_ballots=40,
    ...
    def test_turnout_report_by_gender_sub_constituency_view(self):
        ...
        table_header_texts = [header.text for header in
                              doc.find('thead').findAll('th')]
        self.assertEqual(
            table_header_texts, ['Municipality',
                                 'Race Type',
                                 'Human',
                                 'Voters',
                                 'Registrants',
    ...
    def test_turnout_data_by_gender_region_view(self):
        ...
        response =\
            view(request, tally_id=self.tally.pk, admin_level='region')
        ...
        (area_name_male, sub_race_male, gender_male, voters_male,
         registrants_male, turnout_male) = data[0]
        ...
        self.assertEqual(
            voters_male, '<td class="center">60</td>') # 55 valid + 5 invalid
        ...
        (area_name_female, sub_race_female, gender_female, voters_female,
         registrants_female, turnout_female) =\
    ...
    def test_turnout_data_by_gender_office_view(self):
        ...
        # Male Row Data Check
        (area_name_male, _, gender_male, voters_male,
         registrants_male, turnout_male) = data[0]
    ...
    def test_turnout_data_by_gender_sub_constituency_view(self):
        ...
        """
        request = self.factory.get(
            str(f'/data/turnout-report-by-gender-data/'
        ...
        self.assertEqual(
            area_name_male,
            f'<td class="center">{self.sc.name}</td>') # Sub Constituency Code
        self.assertEqual(gender_male, '<td class="center">Man</td>')
        self.assertEqual(voters_male, '<td class="center">60</td>')
        self.assertEqual(registrants_male, '<td class="center">100</td>')
        self.assertEqual(turnout_male, '<td class="center">60.0</td>')
