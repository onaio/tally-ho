...
class TestIntake(TestBase):
    ...
    def test_intaken_get_no_center(self):
        ...
        self.assertEqual(len(response.context_data['pending_center_forms']),
                         0)
        self.assertEqual(len(response.context_data['intaken_center_forms']),
                         0)
    ...
    def test_check_center_details_get_no_center_in_form(self):
        ...
        self.assertEqual(request.session['station_number'],
                         station.station_number)
    ...
    def test_center_detail_view_get_context(self):
        ...
        self.assertIn('encoded_result_form_intake_start_time',
                      request.session)
    ...
    def test_center_detail_post_invalid_state(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, str('Form not in INTAKE or UNSUBMITTED.'
    ...
    def test_enter_center_post_ballot_mismatch(self):
        ...
        tally = create_tally()
        tally.users.add(self.user)
        electrol_race = create_electrol_race(tally=tally,
        ...
        sub_constituency =\
            create_sub_constituency(tally=tally, ballots=[ballot1])
        center = create_center(
...
class TestIntakePrintedView(TestBase):
    ...
    def test_intake_printed_view_get_clerk_success(self):
        ...
        request.user = self.user
        request.session = {}
        response = self.view(request, resultFormPk=result_form.pk,
    ...
    def test_intake_printed_view_not_found(self):
        ...
        request.user = self.user
        request.session = {}
        response = self.view(request, resultFormPk=9999,
                               tally_id=self.tally.pk)
        self.assertEqual(response.status_code, 200)
