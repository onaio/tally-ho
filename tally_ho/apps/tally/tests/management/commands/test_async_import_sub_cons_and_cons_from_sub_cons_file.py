...
class AsyncImportConsAndSubConstTestCase(TransactionTestCase):
    ...
    def test_async_import_sub_cons_and_cons(self):
        # Prepare test data
        csv_file_path =\
            'tally_ho/libs/tests/fixtures/tally_setup_files/subcons.csv'
        task = async_impt_sub_cons_and_cons_tsk.delay(
                        tally_id=self.tally.id,
                        csv_file_path=csv_file_path,)
        task.wait()

        # Assert that the Constituency and SubConstituency objects are created
        ...
        self.assertGreater(sub_cons.count(), 0)

    def test_import_import_sub_cons_and_cons_with_exception(self):
