...
class AsyncImportCentersTestCase(TransactionTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_async_import_centers(self):
        # Prepare test data
        csv_file_path =\
            'tally_ho/libs/tests/fixtures/tally_setup_files/centers.csv'
        task = async_import_centers_from_centers_file.delay(
                        tally_id=self.tally.id,
                        csv_file_path=csv_file_path,)
        task.wait()

        # Assert that the Center objects are created
        centers = Center.objects.filter(tally=self.tally)
        self.assertGreater(centers.count(), 0)

    def test_async_import_centers_with_exception(self):
