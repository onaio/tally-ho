...
class CenterListDataView(
    ...
    def filter_queryset(self, qs):
        ...
        station_ids = self.request.session.get("station_ids")
        ...
        if not show_inactive or show_inactive.lower() != "true":
            qs = qs.filter(
                center__resultform__station_number=F("station_number"),
...
class CenterListView(
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            region_name = (
                region_id
                and Region.objects.get(tally__id=tally_id, id=region_id).name
            )
        ...
        try:
            sub_constituency_code = (
                sub_constituency_id
                and SubConstituency.objects.get(
                    id=sub_constituency_id, tally__id=tally_id
                ).code
            )
        ...
        if format_ == "csv":
            ...
            # Apply active ballot filter for CSV export
            if not show_inactive or show_inactive.lower() != "true":
                station_list = station_list.filter(
                    center__resultform__station_number=F("station_number"),
                    center__resultform__ballot__active=True,
                ).distinct()
...
def get_centers_stations_list(request):
    ...
    # Check for show_inactive parameter
    show_inactive = request.GET.get(show_inactive_query_param)

    centers_stations_list = Station.objects.filter(center__tally__id=tally_id)

    # Apply active ballot filter
    if not show_inactive or show_inactive.lower() != "true":
        centers_stations_list = centers_stations_list.filter(
            center__resultform__station_number=F("station_number"),
            center__resultform__ballot__active=True,
        ).distinct()

    centers_stations_list = (
        centers_stations_list.annotate(
            ...
            sub_constituency_name=F("sub_constituency__name"),
