...
def duplicates(qs, tally_id=None):
    ...
    """
    dupes = (
        ResultForm.objects.values(
            "center", "ballot", "station_number", "tally__id"
        )
        .annotate(Count("id"))
        .order_by()
...
def audit(tally_id=None):
    ...
    return ResultForm.objects.filter(
        form_state=FormState.AUDIT, tally__id=tally_id
    )
...
def get_result_form_with_duplicate_results(
    ballot=None, tally_id=None, qs=None, show_inactive=False
):
    """Find result forms that have duplicate results within the same ballot.

    This function identifies result forms where there are multiple forms in the
    same ballot with identical vote counts for all candidates, and the
    duplicate_reviewed column is False.

    Args:
        tally_id: The ID of the tally to search within
        ballot: Optional ballot number to filter by specific ballot
        qs: Optional QuerySet of ResultForm objects to search within
    Returns:
        QuerySet of ResultForm objects with duplicate results,
        ordered by ballot
    """
    # Get result forms that haven't been reviewed for duplicates
    base_qs = (
        qs
        if qs
        else ResultForm.objects.filter(
            tally_id=tally_id, duplicate_reviewed=False
        ).exclude(results=None)
    )

    # Filter by active ballot unless show_inactive is True
    if not show_inactive:
        base_qs = base_qs.filter(ballot__active=True)

    if ballot:
        base_qs = base_qs.filter(ballot__number=ballot)

    # Group result forms by ballot and get barcodes
    ballot_groups = (
        base_qs.values("ballot")
        .annotate(barcodes=ArrayAgg("barcode"))
        .filter(barcodes__len__gt=1)
    )

    duplicate_barcodes = []

    for ballot_group in ballot_groups:
        barcodes = ballot_group["barcodes"]

        # Get all results for forms in this ballot
        ballot_results = Result.objects.filter(
            result_form__tally__id=tally_id,
            result_form__barcode__in=barcodes,
            active=True,
        ).values("result_form__barcode", "candidate_id", "votes")

        # Group results by barcode and candidate
        form_votes = {}
        for result in ballot_results:
            barcode = result["result_form__barcode"]
            candidate_id = result["candidate_id"]
            votes = result["votes"]

            if barcode not in form_votes:
                form_votes[barcode] = {}
            form_votes[barcode][candidate_id] = votes

        # Check if any forms have identical vote patterns
        barcode_list = list(form_votes.keys())
        for i, barcode1 in enumerate(barcode_list):
            for barcode2 in barcode_list[i + 1 :]:
                if form_votes[barcode1] == form_votes[barcode2]:
                    # Found duplicate results, add both barcodes
                    if barcode1 not in duplicate_barcodes:
                        duplicate_barcodes.append(barcode1)
                    if barcode2 not in duplicate_barcodes:
                        duplicate_barcodes.append(barcode2)

    # Return the result forms with duplicate results
    return base_qs.filter(barcode__in=duplicate_barcodes).order_by("ballot")


class TalliesView(LoginRequiredMixin, GroupRequiredMixin, TemplateView):
...
class DashboardView(
    ...
    def get(self, *args, **kwargs):
        ...
        kwargs["election_levels"] = set(
            ElectrolRace.objects.filter(tally_id=tally_id).values_list(
...
class CreateResultFormView(
    ...
    def form_valid(self, form):
        ...
        if (
            result_form.center
            or result_form.station_number
            or result_form.ballot
            or result_form.office
        ):
        ...
        form = CreateResultForm(
            post_data, instance=result_form, initial=self.initial
        )
        form.save()
...
class EditResultFormView(
    ...
    def get_success_url(self):
        ...
        return reverse(
            "update-form", kwargs={"tally_id": tally_id, "form_id": form_id}
        )


class RemoveResultFormConfirmationView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
    ...
    def get_object(self, queryset=None):
        return ResultForm.objects.get(
            id=self.kwargs["form_id"], tally__id=self.kwargs["tally_id"]
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "abort_submit" in request.POST:
            ...
            try:
                ...
                return super(RemoveResultFormConfirmationView, self).post(
                    request, *args, **kwargs
                )
...
class FormProgressByFormStateView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    DataTablesMixin,
    ...
    def get(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                remote_url=reverse(
                    "form-progress-by-form-state-data",
...
class DuplicateResultTrackingView(
    LoginRequiredMixin, GroupRequiredMixin, TallyAccessMixin, TemplateView
):
    group_required = groups.SUPER_ADMINISTRATOR
    ...
    def get(self, *args, **kwargs):
        ...
        show_inactive_bool = show_inactive.lower() == "true"

        return self.render_to_response(
            self.get_context_data(
                duplicate_results=get_result_form_with_duplicate_results(
                    tally_id=tally_id, show_inactive=show_inactive_bool
                ),
                tally_id=tally_id,
                show_inactive=show_inactive,
            )
...
class DuplicateResultFormView(
    ...
    def post(self, *args, **kwargs):
        ...
        # Get show_inactive parameter for POST requests
        show_inactive = self.request.GET.get(
            show_inactive_query_param, "false"
        )
        show_inactive_bool = show_inactive.lower() == "true"
        ...
        if "duplicate_reviewed" in post_data:
            ...
            if result_form.form_state != FormState.ARCHIVED:
                reject_reason = _(str("Form has duplicate results."))
            ...
            for results_form_duplicate in results_form_duplicates:
                if results_form_duplicate.form_state != FormState.ARCHIVED:
                    ...
                    results_form_duplicate.user = self.request.user.userprofile
...
class FormDuplicatesView(
    ...
    def get(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                remote_url=reverse(
                    "form-duplicates-data", kwargs={"tally_id": tally_id}
                ),
                tally_id=tally_id,
                export_file_name="form-duplicates",
                server_side=True,
            )
        )


class FormClearanceView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    DataTablesMixin,
    ...
    def get(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                remote_url=reverse(
                    "form-clearance-data", kwargs={"tally_id": tally_id}
                ),
                tally_id=tally_id,
...
class FormAuditView(
    ...
    def get(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                remote_url=reverse(
                    "form-audit-data", kwargs={"tally_id": tally_id}
                ),
                tally_id=tally_id,
...
class FormProgressByFormStateDataView(
    ...
    def filter_queryset(self, qs):
        ...
        keyword = self.request.POST.get("search[value]")
        qs = qs.filter(
            tally__id=tally_id, center__sub_constituency__code__isnull=False
        )
        ...
        for state in FormState.__publicMembers__():
            count_by_form_state_queries[state.name.lower()] = Count(
                "barcode", filter=Q(form_state=state)
            )
        ...
        qs = (
            qs.annotate(sub_con_code=F("center__sub_constituency__code"))
            .values("sub_con_code")
            .annotate(
    ...
    def render_column(self, row, column):
        ...
        if column in self.columns:
            ...
            if (
                isinstance(column, tuple) is False
                and row[column] != 0
                and column in ["unsubmitted", "clearance", "audit"]
            ):
                params = {
                ...
                remote_data_url = reverse(
                    "form-list", kwargs={"tally_id": tally_id}
                )
                ...
                if current_state_form_count > 0:
                    ...
                    current_state_form_query_param_string = urlencode(
                        current_state_form_url_params
                    )
                    current_state_form_remote_data_url = str(
                        f"{remote_data_url}?"
                        f"{current_state_form_query_param_string}"
                    )
                    current_state_column_val = str(
                        "<span>"
                        f"<a href="
                ...
                if unprocessed_count > 0:
                    ...
                    unprocessed_forms_query_param_string = urlencode(
                        unprocessed_forms_url_params
                    )
                    unprocessed_remote_data_url = str(
                        f"{remote_data_url}?"
                        f"{unprocessed_forms_query_param_string}"
                    )
                    unprocessed_column_val = str(
                        "<span>"
            ...
            return str(f'<td class="center">{column_val}</td>')

        else:
            return super(FormProgressByFormStateDataView, self).render_column(
                row, column
            )
...
class FormActionView(
    ...
    def get(self, *args, **kwargs):
        ...
        audits = Audit.objects.filter(
            ...
            resolution_recommendation=AuditResolution.MAKE_AVAILABLE_FOR_ARCHIVE,
            result_form__tally__id=tally_id,
        ).all()

        return self.render_to_response(
            self.get_context_data(audits=audits, tally_id=tally_id)
        )
...
class CreateCenterView(
    ...
    def form_valid(self, form):
        center = form.save()
        self.success_message = _(
            "Successfully Created Center %(center)s" % {"center": center.code}
        )
...
class RemoveCenterConfirmationView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
    ...
    def get_object(self, queryset=None):
        return Center.objects.get(
            code=self.kwargs["center_code"], tally__id=self.kwargs["tally_id"]
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "abort_submit" in request.POST:
            ...
            try:
                return super(RemoveCenterConfirmationView, self).post(
                    request, *args, **kwargs
                )
            except ProtectedError:
                request.session["error_message"] = str(
...
class DisableEntityView(
    ...
    def post(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, tally_id=tally_id)
        )


class EnableEntityView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
...
class CreateElectrolRaceView(
    ...
    def get_context_data(self, **kwargs):
        tally_id = self.kwargs.get("tally_id")
        context = super(CreateElectrolRaceView, self).get_context_data(
            **kwargs
        )
        context["tally_id"] = tally_id
...
class EditElectrolRaceView(
    ...
    def get_object(self, queryset=None):
        ...
        return get_object_or_404(
            ElectrolRace, tally__id=tally_id, id=electrol_race_id
        )
    ...
    def get_success_url(self):
        ...
        return reverse(
            "edit-electrol-race",
            kwargs={"tally_id": tally_id, "id": electrol_race_id},
        )
...
class DisableElectrolRaceView(
    ...
    def get(self, *args, **kwargs):
        ...
        entity_name = f"Electrol Race: {election_level} - {sub_race_type}"
        ...
        return self.render_to_response(
            self.get_context_data(
                form=form, entityName=entity_name, tally_id=tally_id
            )
        )
    ...
    def post(self, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, tally_id=self.tally_id)
        )


class EnableElectrolRaceView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
...
class EnableBallotView(
    ...
    def get(self, *args, **kwargs):
        ...
        messages.add_message(
            self.request, messages.INFO, _("Ballot Successfully enabled.")
        )

        return redirect(self.success_url, tally_id=tally_id)


class CreateStationView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
...
class QuarantineChecksListView(
    ...
    def get_context_data(self, **kwargs):
        context = super(QuarantineChecksListView, self).get_context_data(
            **kwargs
        )
        context["tally_id"] = self.kwargs.get("tally_id")
    ...
    def get(self, *args, **kwargs):
        ...
        return self.render_to_response(self.get_context_data(checks=checks))


class QuarantineChecksConfigView(
    LoginRequiredMixin, GroupRequiredMixin, ReverseSuccessURLMixin, UpdateView
):
    ...
    def get_context_data(self, **kwargs):
        context = super(QuarantineChecksConfigView, self).get_context_data(
            **kwargs
        )
        context["tally_id"] = self.kwargs.get("tally_id")
...
class RemoveStationConfirmationView(
    LoginRequiredMixin,
    GroupRequiredMixin,
    TallyAccessMixin,
    ReverseSuccessURLMixin,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "abort_submit" in request.POST:
            ...
            try:
                ...
                pass
            return super(RemoveStationConfirmationView, self).post(
                request, *args, **kwargs
            )
...
class CreateUserView(
    LoginRequiredMixin, GroupRequiredMixin, TallyAccessMixin, CreateView
):
    group_required = groups.SUPER_ADMINISTRATOR
    template_name = "tally_manager/edit_user_profile.html"
    model = UserProfile
