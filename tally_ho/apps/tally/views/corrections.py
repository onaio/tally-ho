...
def save_result_form_processing_stats(
    ...
    :param result_form: The result form being processed by the corrections
        clerk.
    """
    corrections_start_time = dateutil.parser.parse(encoded_start_time)
    ...
    form_processing_time_in_seconds = (
        corrections_end_time - corrections_start_time
    ).total_seconds()
...
def get_corrections_forms(result_form):
    ...
    candidate_results = result_form_candidate_results(
        result_form, num_results=2
    )
...
def get_recon_form(result_form):
    ...
    if results.count() != 2:
        ...
        if results.count() - final_results.count() == 2:
            ...
            raise SuspiciousOperation(
                _("There should be exactly two reconciliation results.")
            )
    ...
    return [
        (field, recon_form_2[field.name], field.data.__class__.__name__)
...
def recon_form_for_version(results, entry_version):
    return ReconForm(
        data=model_to_dict(results.filter(entry_version=entry_version)[0])
    )
...
def save_recon(post_data, user, result_form):
    ...
    for v1, v2, _type in get_recon_form(result_form):
        if v1.data != v2.data:
            ...
            if value or (v1.name == "notes" and not value):
                if _type == "bool":
...
class CorrectionView(
    ...
    def get_context_data(self, **kwargs):
        ...
        self.request.session["encoded_result_form_corrections_start_time"] = (
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if not result_form.corrections_passed:
                raise Exception(_("Results do not match."))
...
class CorrectionRequiredView(
    ...
    def corrections_response(self, result_form, errors=None):
        tally_id = self.kwargs.get("tally_id")
        recon, candidate_results = get_corrections_forms(result_form)
    ...
    def post(self, *args, **kwargs):
        ...
        if "submit_corrections" in post_data:
            ...
            try:
                ...
                return self.corrections_response(result_form, "%s" % e)
            except SuspiciousOperation as e:
                self.request.session["error_message"] = "%s" % e
            ...
            return redirect(self.success_url, tally_id=tally_id)
        return incorrect_checks(
            post_data, result_form, "corrections", tally_id
        )
