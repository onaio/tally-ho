...
from django.db.models import (
    F, IntegerField, Subquery, OuterRef, Case, When, Value as V, CharField
)
...
def get_overvoted_forms_queryset(tally_id, data=None):
    ...
    Subquery(
        ...
        .values('gender')[:1],
        output_field=IntegerField())
    recon_forms = ReconciliationForm.objects.filter(
    ...
    if data:
        ...
        if sub_con_codes:
            recon_forms =\
                recon_forms.filter(municipality_code__in=sub_con_codes)
        if election_level_names:
            recon_forms = recon_forms.filter(
                race__in=election_level_names
            )
        if sub_race_type_names:
            recon_forms = recon_forms.filter(
                sub_race__in=sub_race_type_names
            )
    forms = [
