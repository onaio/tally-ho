...
def get_candidate_results_queryset(tally_id, data=None):
    ...
    result_forms = (
        ResultForm.objects.select_related(
            "ballot",
            "center",
            "center__office",
    ...
    if data:
        ...
        if election_level_names:
            result_forms = result_forms.filter(
                ballot__electrol_race__election_level__in=election_level_names
            )
    ...
    for result_form in result_forms:
        ...
        for candidate in result_form.ballot.candidates.all():
            ...
            votes = sum(
                result.votes
                for result in candidate.final_results
            ...
            row["candidate_status"] = "enabled" if candidate.active else "disabled"
            queryset.append(row)
...
class CandidateResultsDataView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Filtering
        if search:
            queryset = [row for row in queryset if search.lower() in str(row).lower()]
            total_records = len(queryset)
